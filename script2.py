...
# Replace _imageBytes with _imageBytesList
content = re.sub(r'Uint8List\?\s+_imageBytes;', r'List<Uint8List> _imageBytesList = []; bool _isLoading = false;', content)
...
# NewUploadImageWithPreview -> MultiUploadImageWithPreview
...
# Replace onPressed in the submit button (uploadImage => saveDestination)
content = re.sub(r'onPressed: \(\) \{\s*// Upload gambar lalu simpan data jika valid\s*uploadImage\(\);\s*\},', 
    r'''onPressed: () {
                    saveDestination();
                  },''', content)

# Replace uploadImage and saveDestination
old_block = r'''  // Fungsi untuk mengunggah gambar ke Supabase[\s\S]*?    \} else \{
